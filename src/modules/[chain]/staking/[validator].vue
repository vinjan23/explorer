...
// reversed transactions for display
const reversedTxs = computed(() =>
  txs.value.tx_responses?.slice().reverse() ?? []
);
...
onMounted(() => {
  ...
  blockchain
    .rpc.getDistributionValidatorOutstandingRewards(validator)
    .then((res) => {
  ...
  // pageload and events omitted for performance
});
...
function mapEvents(
  eventsArr: { type: string; attributes: { key: string; value: string }[] }[]
) {
  const attributes = eventsArr
    ...
    .map((x) => {
      ...
      if (x.attributes.some((a) => a.key === 'amount')) {
        ...
        x.attributes.forEach((attr) => {
          output[
            uint8ArrayToString(fromBase64(attr.key))
          ] = uint8ArrayToString(fromBase64(attr.value));
        });
      }
      return output;
    });
  ...
  <div>
          ...
          <div class="m-4 text-sm">
            ...
            <div class="card-list">
              <div class="flex items-center mb-2">
                ...
                <span>{{ String(v.status).replace('BOND_STATUS_', '') }}</span>
              </div>
              <div class="flex items-center">
            ...
            <div class="card-list">
              ...
              <div class="flex items-center">
                ...
                <span>{{ format.formatToken({ amount: v.liquid_shares, denom: staking.params.bond_denom }, false) }}</span>
              </div>
            </div>
          </div>
        </div>
      ...
      <div class="bg-base-100 rounded shadow overflow-x-auto">
        ...
        <div class="px-4 pb-4">
          ...
          <div class="mb-3">
            ...
            <div class="text-xs">{{ v.operator_address }}</div>
          </div>
          <div class="mb-3">
    ...
    <div v-if="delegations.delegation_responses" class="mt-5 bg-base-100 shadow rounded p-4">
      <div class="text-lg mb-4 font-semibold">
        ...
        <span class="float-right">
          {{ delegations.delegation_responses.length }} / {{ delegations.pagination?.total }}
        </span>
      </div>
        ...
        <table class="table validatore-table w-full">
          ...
          <tbody>
            <tr v-for="{ balance, delegation } in delegations.delegation_responses" :key="delegation.delegator_address">
              <td class="text-sm text-primary">{{ delegation.delegator_address }}</td>
              <td class="truncate text-primary">{{ format.formatToken(balance) }}</td>
            </tr>
          </tbody>
        </table>
        <PaginationBar
    ...
    <div class="mt-5 bg-base-100 shadow rounded p-4">
      ...
      <div class="rounded overflow-auto">
        <table class="table validatore-table w-full">
          <thead>
            <th class="text-left pl-4"> {{ $t('account.height') }} </th>
          ...
          <tbody>
            <tr v-for="item in reversedTxs" :key="item.txhash">
              ...
              <td class="truncate text-primary" style="max-width: 200px">
                <RouterLink :to="`/${props.chain}/tx/${item.txhash}`">{{ item.txhash }}</RouterLink>
              </td>
              <td>
                <div class="flex items-center">
    ...
    <div class="mt-5 bg-base-100 shadow rounded p-4">
      ...
      <div class="rounded overflow-auto">
        <table class="table validatore-table w-full">
          ...
          <tbody>
            <tr v-for="item in events.tx_responses" :key="item.txhash">
    ...
    <div class="toast" v-show="showCopyToast === 1">
      <div class="alert alert-success">
        <div class="text-xs md:!text-sm">{{ tipMsg.msg }}</div>
      </div>
    </div>
